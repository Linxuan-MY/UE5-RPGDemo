...
void UHeroGameplayAbility_PickUpStones::CollectStones()
{
	...
	{
		if (ARPGDemoStoneBase* FoundStone = Cast<ARPGDemoStoneBase>(TraceHit.GetActor()))
		{
...
void UHeroGameplayAbility_PickUpStones::ConsumeStones()
{
	...
	for (ARPGDemoStoneBase* CollectedStone : CollectedStones)
	{
		if (CollectedStone)
		{
			CollectedStone->Consume(GetRPGDemoAbilitySystemComponentFromActorInfo(), GetAbilityLevel());
		}
	}
}
